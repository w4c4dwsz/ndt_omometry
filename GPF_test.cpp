...
#include <cmath>
bool my_fun(pcl::PointXYZ a, pcl::PointXYZ b){
        return a.z < b.z;
    }
...
int main()
{
    ...
    int div_segment_num = 6; // number of the segment used to dividing pointcloud
    double N_LPR = 0.3; // number of points used to estimate the LPR(lowest point representative)
    double Th_seeds = 0.1; // threshold for points to be considered initial seeds
    double Th_dist = 0.2; // threshold distance frome plane 
    ...
    // cout << "===" << endl;
    // cout << pcd_filename_v.back() << endl;
    // cout << pcd_filename_v.back().size() << endl;
    // cout << "===" << endl;
    ...
    while(!pcd_filename_v.empty() && !viewer.wasStopped())
    {
        ...
        cout << "use time = " << time.count() << endl;
        // cout << "filtered ground point number = " << all_cloud_ground->points.size() <<endl;
        // cout << "not ground point number = " << all_cloud_not_ground->points.size() <<endl;
        // pcl::visualization::PointCloudColorHandlerRandom<pcl::PointXYZ> all_cloud_not_ground_color_h(all_cloud_not_ground);
        // pcl::visualization::PointCloudColorHandlerRandom<pcl::PointXYZ> all_cloud_ground_color_h(all_cloud_ground);
